...
def test_dashboard_contains_operator_pattern_library_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "build_operator_pattern_records", lambda **kwargs: [
        {
            "pattern_id": "pattern-device-aabb",
            "pattern_type": "device",
            "title": "Recurring device pattern",
            "match_signals": ["queue:in_review"],
            "risk_profile": "medium",
    ...
    monkeypatch.setattr(dashboard, "match_scopes_to_patterns", lambda scopes, patterns: [
        {
            ...
            "scope_id": "AA:BB:CC:DD:EE:01",
            "pattern_id": "pattern-device-aabb",
            "pattern_title": "Recurring device pattern",
            "match_score": 0.87,
            "confidence_level": "high",
            "risk_profile": "medium",
            "recommended_playbooks": ["pb-critical-pack"],
    ...
    monkeypatch.setattr(dashboard, "summarize_operator_pattern_library", lambda patterns, matches=None: {
        ...
        "likely_matches": matches or [],
        "pattern_based_guidance": ["device:AA:BB:CC:DD:EE:01 -> pattern-device-aabb | use pb-critical-pack"],
    })

    html = dashboard.render_dashboard_html(SAMPLE_DEVICES, "2026-04-18 18:00:00")

    assert "Operator Pattern Library / Recurring Case Memory" in html
    assert "Known patterns" in html
    assert "Recurring case types" in html
    assert "Likely matches" in html
    assert "Pattern-based guidance" in html
