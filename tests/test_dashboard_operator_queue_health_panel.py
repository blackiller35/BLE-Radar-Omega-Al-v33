...
def test_dashboard_contains_operator_queue_health_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "build_operator_briefing", lambda **kwargs: {
        ...
        "suggested_next_steps": ["Review queue health"],
    })
    ...
    monkeypatch.setattr(dashboard, "summarize_queue_health", lambda snapshot, items: {
        ...
        "aging_overview": [
            {"bucket": "fresh", "count": 0},
            {"bucket": "warm", "count": 0},
            {"bucket": "aging", "count": 0},
            {"bucket": "stale", "count": 1},
        ],
        "blocked_items": items,
        "stale_items": items,
        "operator_pressure": {
    ...
    assert "Operator Queue Health / Aging / Bottlenecks" in html
    assert "Queue health" in html
    assert "Aging overview" in html
    assert "Blocked items" in html
    assert "Stale items" in html
    assert "Operator pressure" in html
