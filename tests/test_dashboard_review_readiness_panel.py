...
def test_dashboard_contains_review_readiness_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "build_operator_briefing", lambda **kwargs: {
        ...
        "suggested_next_steps": ["Review readiness gate"],
    })
    ...
    monkeypatch.setattr(dashboard, "build_review_readiness_profiles", lambda *args, **kwargs: [
        {
            "review_id": "review-device-aabb-1",
            "scope_type": "device",
            "scope_id": "AA:BB:CC:DD:EE:01",
    ...
    assert "Review Readiness / Readiness Gate" in html
    assert "Review readiness" in html
    assert "Ready for review" in html
    assert "Needs more evidence" in html
    assert "Ready for handoff" in html
    assert "Ready for archive" in html
