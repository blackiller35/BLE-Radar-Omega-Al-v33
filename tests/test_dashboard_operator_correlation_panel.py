...
def test_dashboard_contains_operator_correlation_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "build_operator_briefing", lambda **kwargs: {
        ...
        "suggested_next_steps": ["Review cluster links"],
    })
    ...
    monkeypatch.setattr(dashboard, "summarize_clusters", lambda clusters: {
        "top_correlation_clusters": clusters,
        "possible_coordinated_devices": clusters,
        "needs_cluster_review": clusters,
    })

    html = dashboard.render_dashboard_html(SAMPLE_DEVICES, "2026-04-18_10-30-00")

    assert "Correlation Clusters / Campaign View" in html
    assert "Top correlation clusters" in html
    assert "Possible coordinated devices" in html
    assert "Needs cluster review" in html
