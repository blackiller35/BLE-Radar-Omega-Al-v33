...
def test_dashboard_contains_operator_outcomes_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "build_operator_briefing", lambda **kwargs: {
        ...
        "suggested_next_steps": ["Review outcomes"],
    })
    ...
    assert "Operator Outcomes / Feedback Loop" in html
    assert "Operator outcomes" in html
    assert "Most effective actions" in html
    assert "Reopened items" in html
