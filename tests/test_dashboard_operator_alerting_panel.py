...
def test_dashboard_contains_operator_alerting_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "summarize_alerts", lambda alerts, recent_log_events=None: {
        ...
        "recent_escalations": alerts,
        "needs_immediate_review": alerts,
    })

    html = dashboard.render_dashboard_html(SAMPLE_DEVICES, "2026-04-18_10-30-00")

    assert "Operator Alerting & Escalation" in html
    assert "Active alerts" in html
    assert "Recent escalations" in html
    assert "Needs immediate review" in html
