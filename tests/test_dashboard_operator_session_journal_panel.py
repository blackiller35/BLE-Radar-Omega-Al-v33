...
def test_dashboard_contains_operator_session_journal_panel(monkeypatch):
    ...
    assert "Operator Session Journal / Shift Continuity" in html
    assert "Current session journal" in html
    assert "Shift activity" in html
    assert "Carry over items" in html
    assert "Next shift priorities" in html
    assert "Recent handoffs" in html
