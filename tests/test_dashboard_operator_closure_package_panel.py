...
def test_dashboard_contains_operator_closure_package_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "summarize_operator_escalation_feedback", lambda rows: {
        "escalation_feedback": [],
    ...
    monkeypatch.setattr(dashboard, "build_operator_closure_packages", lambda *args, **kwargs: [
        {
            ...
            "key_supporting_signals": ["review_result=close_as_resolved"],
    ...
    monkeypatch.setattr(dashboard, "summarize_operator_closure_packages", lambda rows: {
        "closure_packages": rows,
        "recently_closed": rows,
        "closed_after_escalation": [],
        "resolved_vs_false_positive": {"resolved": 1, "false_positive": 0},
        "followup_still_needed": [],
    })

    html = dashboard.render_dashboard_html(SAMPLE_DEVICES, "2026-04-18 18:00:00")

    assert "Operator Closure Packages / Final Resolution" in html
    assert "Closure packages" in html
    assert "Recently closed" in html
    assert "Closed after escalation" in html
    assert "Resolved vs false positive" in html
    assert "Follow-up still needed" in html
