...
def test_dashboard_contains_operator_escalation_feedback_panel(monkeypatch):
    ...
    monkeypatch.setattr(dashboard, "build_operator_escalation_feedback_records", lambda *args, **kwargs: [
        {
            ...
            "decision_summary": "specialist confirmed",
            "specialist_notes": "notes",
            "requested_followup": ["apply specialist guidance"],
    ...
    monkeypatch.setattr(dashboard, "summarize_operator_escalation_feedback", lambda rows: {
        "escalation_feedback": rows,
        "returned_for_followup": rows,
        "specialist_decisions": rows,
        "ready_to_close": [],
        "needs_more_data": [],
    })
    ...
    assert "Operator Escalation Feedback / Specialist Return" in html
    assert "Escalation feedback" in html
    assert "returned for follow-up" in html
    assert "Specialist decisions" in html
